...
> **Welcome to Product-Track Week 5, Hotfix 1!**
...
1. **Read the comments** at the top of `loadBalancer.js` - they describe the problem
...
| File | Purpose |
|------|---------|
| `loadBalancer.js` | The code with bugs - **fix this file** |
...
### Bug #1
**What's wrong:** Using Math.random() makes the hash non-deterministic

**How to find it:** Search for `BUG` in `loadBalancer.js` - the comments around each bug explain what's broken.

### Bug #2
**What's wrong:** Session affinity broken because hash is non-deterministic

**How to find it:** Search for `BUG` in `loadBalancer.js` - the comments around each bug explain what's broken.

### Bug #3
**What's wrong:** Missing session affinity cookie update â€” orphaned sessions get 503
...
1. **Read the top comment block** in `loadBalancer.js` carefully - it has:
